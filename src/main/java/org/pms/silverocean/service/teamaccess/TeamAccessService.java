...
public class TeamAccessService {
    ...
    public TeamAccessModels.InvitationView invite(TeamAccessModels.InviteRequest request) {
        ...
        access = new AccessContext(lockedWorkspace, access.owner(), access.privilegeLevel(), access.actorRole(),
                access.actorScopeType(), access.allowedResourceIds());
        ...
        List<Long> resources = validateScope(access, request.scopeType(), request.resourceIds());
    ...
    public TeamAccessModels.InvitationView resend(long invitationId) {
        ...
        requireWithinActorScope(access, invitation.getScopeType(), readIds(invitation.getResourceIdsJson()));
    ...
    public TeamAccessModels.MemberView updateScope(long memberId, TeamAccessModels.ScopeUpdate request) {
        ...
        requireCanManage(access, member); List<Long> ids = validateScope(access, request.scopeType(), request.resourceIds());
    ...
    private AccessContext accessContext(){Users user=users.getUserObject();if(user==null)throw invalid();PMSRole active=users.getActiveRole();try{TeamBusinessArea area=TeamBusinessArea.fromOwnerRole(active);CustomerWorkspace workspace=workspaces.findByOwnerUserIdAndBusinessAreaAndActiveTrue(user.getId(),area).orElseGet(()->createWorkspace(user,area));return new AccessContext(workspace,true,100,null,TeamScopeType.ENTIRE_WORKSPACE,Set.of());}catch(IllegalArgumentException ignored){WorkspaceMembership member=workspaceSelection.selectedMembership(user.getId()).orElseThrow(this::invalid);if(member.getMembershipRole()!=TeamMembershipRole.WORKSPACE_ADMIN)throw new PMSCustomException(ResponseCode.FORBIDDEN_ACCESS);CustomerWorkspace workspace=workspaces.findById(member.getWorkspaceId()).filter(CustomerWorkspace::isActive).orElseThrow(this::invalid);return new AccessContext(workspace,false,member.getMembershipRole().privilegeLevel(),member.getMembershipRole(),member.getScopeType(),Set.copyOf(readIds(member.getResourceIdsJson())));}}
    ...
    private void send(WorkspaceInvitation invitation, CustomerWorkspace workspace, String rawToken) {
        String link = formatInviteLink(
                config.getConfigByName(PMSConfigs.INVITE_LINK_URL).get().stringValue(), rawToken);
        String role = roleName(invitation.getRoleDefinitionId(), invitation.getMembershipRole());
    ...
    private TeamAccessModels.WorkspaceView view(AccessContext a){CustomerWorkspace w=a.workspace();List<TeamAccessModels.RoleOption>roleOptions=roleDefinitions.findByBusinessAreaAndActiveTrueOrderByDisplayName(w.getBusinessArea()).stream().filter(d->a.owner()||a.privilegeLevel()>d.getPermissionTemplate().privilegeLevel()).map(d->new TeamAccessModels.RoleOption(d.getId(),d.getCode(),d.getDisplayName(),d.getDescription(),d.getPermissionTemplate())).toList();List<IdNameDescDTO>resources=properties.findAllByCreatedByAndActiveTrue(w.getOwnerUserId()).stream().filter(p->a.owner()||a.actorScopeType()==TeamScopeType.ENTIRE_WORKSPACE||a.allowedResourceIds().contains(p.getId())).map(p->new IdNameDescDTO(p.getId(),p.getName(),p.getAddress())).toList();List<TeamAccessModels.InvitationView>inviteViews=invitations.findByWorkspaceIdAndActiveTrueOrderByCreatedOnDesc(w.getId()).stream().filter(i->i.getMembershipId()==null).filter(i->visibleToActor(a,i.getScopeType(),readIds(i.getResourceIdsJson()))).map(this::invitationView).toList();List<TeamAccessModels.MemberView>memberViews=memberships.findByWorkspaceIdAndActiveTrueOrderByCreatedOnDesc(w.getId()).stream().filter(m->visibleToActor(a,m.getScopeType(),readIds(m.getResourceIdsJson()))).map(this::memberView).toList();long used=invitations.countByWorkspaceIdAndStatusInAndActiveTrue(w.getId(),List.of(TeamMembershipStatus.PENDING))+memberships.countByWorkspaceIdAndStatusInAndActiveTrue(w.getId(),OCCUPIED_MEMBER_STATUSES);return new TeamAccessModels.WorkspaceView(w.getId(),w.getName(),w.getBusinessArea(),a.owner(),a.owner()||a.actorScopeType()==TeamScopeType.ENTIRE_WORKSPACE,seatLimit(w),used,roleOptions,resources,inviteViews,memberViews);}
    ...
    private record RawToken(String raw,String hash){} private record AccessContext(CustomerWorkspace workspace,boolean owner,int privilegeLevel,TeamMembershipRole actorRole,TeamScopeType actorScopeType,Set<Long> allowedResourceIds){}
}
