...
public class AffiliateAdminService {
    ...
    public record Edit(@NotBlank @Pattern(regexp="ACTIVE|SUSPENDED|BLACKLISTED|INACTIVE") String status,
    ...
    public Page<Summary> directory(String query, String status, Pageable pageable) {
        requireAdmin();
        var page = queries.directory(search(query), state(status, Set.of("ACTIVE","SUSPENDED","BLACKLISTED","INACTIVE")), bounded(pageable));
        Map<Long,Users> identities = identities(page.getContent().stream().map(AffiliateProfile::getUserId).toList());
        return page.map(p -> summary(p, identities.get(p.getUserId())));
    }
    ...
    private String statusNotice(String status) {return switch(status) {
        case "ACTIVE" -> "Your affiliate programme access is active. Existing commission entries have not been recalculated.";
