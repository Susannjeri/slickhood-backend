...
public final class WealthAnalytics {
    ...
    private static List<Insight> insights(List<WealthAsset> assets,List<AssetPerformance> rows,List<WealthObligation> obligations){
        ...
        for(AssetPerformance row:rows){
            ...
            if(row.cashFlow().signum()<0) result.add(new Insight("HIGH","NEGATIVE_CASH_FLOW","Negative cash flow",row.name()+" has negative cash flow after scheduled debt payments.",row.assetId(),"Review rent, vacancy and controllable operating costs."));
        ...
        for(WealthAsset asset:assets)if(asset.getValuationDate()==null||ChronoUnit.DAYS.between(asset.getValuationDate(),today)>365)result.add(new Insight("MEDIUM","STALE_VALUATION","Valuation needs review",asset.getName()+" has not been valued for more than 12 months.",asset.getId(),"Add a current professional or market valuation."));
    ...
    private static LocalDate deadline(WealthObligation o){if(o.getDueDate()==null)return o.getExpiryDate();if(o.getExpiryDate()==null)return o.getDueDate();return o.getDueDate().isBefore(o.getExpiryDate())?o.getDueDate():o.getExpiryDate();}
