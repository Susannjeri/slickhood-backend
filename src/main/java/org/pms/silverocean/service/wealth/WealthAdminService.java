...
public class WealthAdminService {
 ...
 @Transactional public WealthAssetType update(long id,AssetTypeRequest r){WealthAssetType t=typeRepo.findById(id).orElseThrow(this::notFound);if(r.code()!=null&&!t.getCode().equalsIgnoreCase(code(r.code())))throw invalid();apply(t,r);if(Boolean.FALSE.equals(r.active())&&t.isActive()&&typeRepo.countByActiveTrue()<=1)throw invalid();if(r.active()!=null)t.setActive(r.active());return typeRepo.save(t);}
