...
public class WealthService {
    ...
    public List<VaultDocumentView> documents(){return vaultRepo.findAllByOwnerUserIdAndActiveTrueOrderByCreatedOnDesc(userDao.getUserId()).stream().map(d->new VaultDocumentView(d,garageService.getPresignedUrl(d.getFileRef()))).toList();}
    public VaultDocumentView document(long id){WealthVaultDocument d=vaultRepo.findByIdAndOwnerUserIdAndActiveTrue(id,userDao.getUserId()).orElseThrow(this::notFound);return new VaultDocumentView(d,garageService.getPresignedUrl(d.getFileRef()));}
