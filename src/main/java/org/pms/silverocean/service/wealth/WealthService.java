...
public class WealthService {
    ...
    @Transactional public WealthValuation addValuation(long assetId,ValuationRequest r){if(r.valuationDate().isAfter(LocalDate.now()))throw invalid();WealthAsset a=asset(assetId);if(a.getValuationDate()==null||!r.valuationDate().isBefore(a.getValuationDate())){a.setCurrentValue(r.amount());a.setValuationDate(r.valuationDate());a.setQuoteStatus("MANUAL_OVERRIDE");assetRepo.save(a);}return recordValuation(a,r.amount(),r.valuationDate(),r.source(),r.notes());}
    ...
    @Transactional public VaultDocumentView upload(Long assetId,String category,LocalDate documentDate,LocalDate expiryDate,String notes,MultipartFile file) throws IOException {
        ...
        if(documentDate!=null&&documentDate.isAfter(LocalDate.now())||expiryDate!=null&&documentDate!=null&&expiryDate.isBefore(documentDate))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA);
