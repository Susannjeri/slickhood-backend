...
public class WealthService {
    ...
    private static final Set<String> ASSET_TYPES=Set.of("PROPERTY","LAND","LISTED_SECURITY","INVESTMENT","FUND","GOVERNMENT_SECURITY","SACCO","PENSION","CASH","BUSINESS","VEHICLE","DIGITAL_ASSET","OTHER");
    ...
    @Transactional public WealthLiability addLiability(long assetId,LiabilityRequest r){asset(assetId);if(r.outstandingPrincipal().compareTo(r.originalPrincipal())>0||r.maturityDate()!=null&&r.startDate()!=null&&r.maturityDate().isBefore(r.startDate()))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA);WealthLiability l=new WealthLiability();l.setAssetId(assetId);l.setLender(r.lender().trim());l.setCurrency(r.currency().toUpperCase());l.setOriginalPrincipal(r.originalPrincipal());l.setOutstandingPrincipal(r.outstandingPrincipal());l.setAnnualInterestRate(r.annualInterestRate());l.setMonthlyPayment(r.monthlyPayment());l.setStartDate(r.startDate());l.setMaturityDate(r.maturityDate());stamp(l);return liabilityRepo.save(l);}
    @Transactional public WealthObligation addObligation(long assetId,ObligationRequest r){asset(assetId);if(r.dueDate()==null&&r.expiryDate()==null)throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA);if(r.expiryDate()!=null&&r.effectiveDate()!=null&&r.expiryDate().isBefore(r.effectiveDate()))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA);WealthObligation o=new WealthObligation();o.setAssetId(assetId);o.setObligationType(r.obligationType().trim().toUpperCase(Locale.ROOT));o.setTitle(r.title().trim());o.setEffectiveDate(r.effectiveDate());o.setDueDate(r.dueDate());o.setExpiryDate(r.expiryDate());o.setAmount(r.amount());o.setCurrency(r.currency()==null?null:r.currency().toUpperCase());o.setReminderDays(Optional.ofNullable(r.reminderDays()).orElse(30));o.setNotes(r.notes()==null?null:r.notes().trim());o.setStatus("OPEN");stamp(o);return obligationRepo.save(o);}
    ...
    @Transactional public VaultDocumentView upload(Long assetId,String category,LocalDate documentDate,LocalDate expiryDate,String notes,MultipartFile file) throws IOException {
        if(assetId!=null)asset(assetId);String normalized=category==null?null:category.trim().toUpperCase();if(!VAULT_CATEGORIES.contains(normalized))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA);
