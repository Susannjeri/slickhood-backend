...
public class WealthService {
    ...
    @Transactional public AssetView updateAsset(long id,AssetRequest r){validateAssetRequest(r);WealthAsset a=asset(id);if(!a.getCurrency().equalsIgnoreCase(r.currency()))throw invalid();validatePropertyLink(r.propertyId(),id);BigDecimal old=a.getCurrentValue();LocalDate oldDate=a.getValuationDate();apply(a,r);a=assetRepo.save(a);if(old==null||old.compareTo(r.currentValue())!=0||!Objects.equals(oldDate,r.valuationDate()))recordValuation(a,r.currentValue(),r.valuationDate(),"ASSET_UPDATE","Value updated from asset record");return new AssetView(a);}
