...
public class WealthService {
    ...
    private Map<Long,OperatingInput> operatingInputs(List<WealthAsset> assets){
        ...
        Map<Long,OperatingInput> result=new HashMap<>();for(WealthAsset asset:assets){if(asset.getPropertyId()==null)continue;List<Unit> propertyUnits=units.getOrDefault(asset.getPropertyId(),List.of());BigDecimal arrears=invoices.getOrDefault(asset.getPropertyId(),List.of()).stream().filter(i->"RENTAL".equals(i.getBillingType())&&i.getDueDate()!=null&&i.getDueDate().isBefore(LocalDate.now())&&i.getPendingAmount()>0).map(i->base(BigDecimal.valueOf(i.getPendingAmount()),i.getCurrency())).reduce(BigDecimal.ZERO,BigDecimal::add);result.put(asset.getId(),new OperatingInput(propertyUnits.size(),(int)propertyUnits.stream().filter(Unit::isOccupied).count(),arrears));}return result;
    }
}
