...
public class WealthService {
    ...
    @Transactional public AssetView updateAsset(long id,AssetRequest r){validateAssetRequest(r);WealthAsset a=asset(id);if(!a.getCurrency().equalsIgnoreCase(r.currency()))throw invalid();validatePropertyLink(r.propertyId(),id);BigDecimal old=a.getCurrentValue();LocalDate oldDate=a.getValuationDate();if(oldDate!=null&&r.valuationDate().isBefore(oldDate))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA,"An asset edit cannot replace a newer valuation. Add earlier values through valuation history.");apply(a,r);a=assetRepo.save(a);if(old==null||old.compareTo(r.currentValue())!=0||!Objects.equals(oldDate,r.valuationDate()))recordValuation(a,r.currentValue(),r.valuationDate(),"ASSET_UPDATE","Value updated from asset record");return new AssetView(a);}
    @Transactional public void archiveAsset(long id){WealthAsset a=asset(id);a.setActive(false);assetRepo.save(a);}
    @Transactional public WealthValuation addValuation(long assetId,ValuationRequest r){if(r.valuationDate().isAfter(today()))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA,"Valuation date cannot be in the future.");WealthAsset a=asset(assetId);if(a.getValuationDate()==null||!r.valuationDate().isBefore(a.getValuationDate())){a.setCurrentValue(r.amount());a.setValuationDate(r.valuationDate());a.setQuoteStatus("MANUAL_OVERRIDE");assetRepo.save(a);}return recordValuation(a,r.amount(),r.valuationDate(),r.source(),r.notes());}
    ...
    @Transactional public WealthCashFlow addCashFlow(long assetId,CashFlowRequest r){asset(assetId);if(r.entryDate().isAfter(today()))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA,"Entry date cannot be in the future.");WealthCashFlow f=new WealthCashFlow();f.setAssetId(assetId);apply(f,r);stamp(f);return cashFlowRepo.save(f);}
    @Transactional public WealthCashFlow updateCashFlow(long id,CashFlowRequest r){if(r.entryDate().isAfter(today()))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA,"Entry date cannot be in the future.");WealthCashFlow f=cashFlow(id);apply(f,r);return cashFlowRepo.save(f);}
    ...
    @Transactional public WealthGoal addGoal(GoalRequest r){validateGoalDate(r);WealthGoal g=new WealthGoal();g.setOwnerUserId(userDao.getUserId());apply(g,r);g.setStatus("ACTIVE");stamp(g);return goalRepo.save(g);}
    public GoalView goalDetails(long id){return new GoalView(goal(id));}
    @Transactional public WealthGoal updateGoal(long id,GoalRequest r){validateGoalDate(r);WealthGoal g=goal(id);apply(g,r);return goalRepo.save(g);}
    ...
    @Transactional public VaultDocumentView upload(Long assetId,String category,LocalDate documentDate,LocalDate expiryDate,String notes,MultipartFile file) throws IOException {
        if(assetId!=null)asset(assetId);String normalized=category==null?null:category.trim().toUpperCase();if(normalized==null||!VAULT_CATEGORIES.contains(normalized))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA);
        if(documentDate!=null&&documentDate.isAfter(today()))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA,"Document date cannot be in the future.");
        if(expiryDate!=null&&documentDate!=null&&expiryDate.isBefore(documentDate))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA,"Expiry date cannot be before the document date.");
        if(file==null||file.isEmpty()||file.getSize()>MAX_VAULT_BYTES)throw new PMSCustomException(ResponseCode.UNSUPPORTED_MEDIA_TYPE);if(notes!=null&&notes.length()>500)throw invalid();byte[] bytes=file.getBytes();validateFile(file,bytes);VaultMalwareScanner.Result scan=malwareScanner.scan(bytes);if(scan==VaultMalwareScanner.Result.INFECTED)throw new PMSCustomException(ResponseCode.UPLOAD_MALWARE_DETECTED);if(scan!=VaultMalwareScanner.Result.CLEAN)throw new PMSCustomException(ResponseCode.UPLOAD_SCAN_UNAVAILABLE);String original=file.getOriginalFilename();if(original==null||original.isBlank())throw invalid();if(original.contains("/")||original.contains("\\")||original.chars().anyMatch(Character::isISOControl))throw invalid();String safe=java.nio.file.Path.of(original).getFileName().toString();if(!safe.equals(original)||safe.length()>255)throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA);
    ...
    public Dashboard dashboard(int years,BigDecimal valueGrowth,BigDecimal incomeGrowth,BigDecimal expenseGrowth){
        List<WealthAsset> sourceAssets=ownedAssets();List<Long> ids=sourceAssets.stream().map(WealthAsset::getId).toList();LocalDate today=today();
    ...
    private void apply(WealthAsset a,AssetRequest r){String type=r.assetType().toUpperCase();WealthAssetType configuredType=wealthAdminService.requireForAsset(type,a.getAssetType());String pricing=Optional.ofNullable(r.pricingMode()).orElse("MANUAL");if("MARKET".equals(pricing)&&(!configuredType.isMarketPricingAllowed()||r.instrumentSymbol()==null||r.instrumentSymbol().isBlank()||r.exchangeCode()==null||r.exchangeCode().isBlank()||r.quantity()==null))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA);if("MARKET".equals(pricing)&&!marketQuoteProvider.supports(r.exchangeCode(),r.currency()))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA,"Automated market pricing does not support this exchange and currency. Use manual valuation instead.");a.setPropertyId(r.propertyId());a.setAssetType(type);a.setName(r.name().trim());a.setReference(r.reference());a.setLocation(r.location());a.setCurrency(r.currency().toUpperCase());BigDecimal cost=r.acquisitionCost();if(cost==null&&r.quantity()!=null&&r.averageUnitCost()!=null)cost=r.quantity().multiply(r.averageUnitCost());a.setAcquisitionCost(Optional.ofNullable(cost).orElse(BigDecimal.ZERO));a.setAcquisitionDate(r.acquisitionDate());a.setCurrentValue(r.currentValue());a.setValuationDate(r.valuationDate());a.setStatus(r.status()==null?"ACTIVE":r.status().toUpperCase());a.setExchangeCode(upper(r.exchangeCode()));a.setInstrumentSymbol(upper(r.instrumentSymbol()));a.setQuantity(r.quantity());a.setAverageUnitCost(r.averageUnitCost());a.setPricingMode(pricing);if(!"MARKET".equals(pricing)){a.setMarketPrice(null);a.setQuoteProvider(null);a.setQuoteStatus(null);a.setQuoteAsOf(null);}}
    ...
    private void apply(WealthGoal g,GoalRequest r){if(r.targetAmount().signum()==0&&!"DEBT_REDUCTION".equals(r.goalType()))throw invalid();g.setGoalType(r.goalType().trim().toUpperCase());g.setName(r.name().trim());g.setTargetAmount(r.targetAmount());g.setCurrency(r.currency().toUpperCase());g.setTargetDate(r.targetDate());}
    ...
    private void validateAssetRequest(AssetRequest r){LocalDate today=today();if(r.valuationDate().isAfter(today))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA,"Valuation date cannot be in the future.");if(r.acquisitionDate()!=null&&r.acquisitionDate().isAfter(today))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA,"Acquisition date cannot be in the future.");}
    private void validateGoalDate(GoalRequest r){if(r.targetDate().isBefore(today()))throw new PMSCustomException(ResponseCode.INVALID_FIELD_DATA,"Target date cannot be in the past.");}
    private LocalDate today(){return LocalDate.now(PMSUtils.getZoneId());}
    ...
    private void validateFile(MultipartFile file,byte[] bytes){if(file==null||file.isEmpty()||file.getSize()>MAX_VAULT_BYTES||file.getContentType()==null||!ALLOWED_TYPES.contains(file.getContentType())||!signatureMatches(file.getContentType(),bytes))throw new PMSCustomException(ResponseCode.UNSUPPORTED_MEDIA_TYPE);}
    ...
    private Map<Long,OperatingInput> operatingInputs(List<WealthAsset> assets){
        List<Long> propertyIds=assets.stream().map(WealthAsset::getPropertyId).filter(Objects::nonNull).distinct().filter(id->propertyRepo.findByIdAndStaffOrOwner(id,userDao.getUserId()).isPresent()).toList();if(propertyIds.isEmpty())return Map.of();
        ...
        LocalDate today=today();Map<Long,OperatingInput> result=new HashMap<>();for(WealthAsset asset:assets){if(asset.getPropertyId()==null)continue;List<Unit> propertyUnits=units.getOrDefault(asset.getPropertyId(),List.of());BigDecimal arrears=invoices.getOrDefault(asset.getPropertyId(),List.of()).stream().filter(i->"RENTAL".equals(i.getBillingType())&&i.getDueDate()!=null&&i.getDueDate().isBefore(today)&&i.getPendingAmount()>0).map(i->base(BigDecimal.valueOf(i.getPendingAmount()),i.getCurrency())).reduce(BigDecimal.ZERO,BigDecimal::add);result.put(asset.getId(),new OperatingInput(propertyUnits.size(),(int)propertyUnits.stream().filter(Unit::isOccupied).count(),arrears));}return result;
    }
}
