...
public interface MaintenanceWorkOrderRepo extends JpaRepository<MaintenanceWorkOrder,Long>{
 ...
 @Query("SELECT w FROM MaintenanceWorkOrder w JOIN Unit u ON u.id=w.unitId WHERE w.active AND w.unitId=:unitId AND (w.requestedByUserId=:userId OR u.createdBy=:userId OR EXISTS (SELECT 1 FROM PropertyManager pm WHERE pm.propertyId=u.propertyId AND pm.userId=:userId AND pm.active) OR EXISTS (SELECT 1 FROM PropertyOwnership po WHERE po.propertyId=u.propertyId AND (po.unitId IS NULL OR po.unitId=u.id) AND po.homeownerUserId=:userId AND po.active) OR EXISTS (SELECT 1 FROM ProviderService s, ProviderProfile pp WHERE s.id=w.assignedProviderServiceId AND s.profileId=pp.id AND s.active AND pp.active AND pp.userId=:userId)) ORDER BY w.createdOn DESC")
 List<MaintenanceWorkOrder> findAccessibleByUnit(long unitId,long userId);
 @Query("SELECT w FROM MaintenanceWorkOrder w JOIN Unit u ON u.id=w.unitId WHERE w.active AND w.id=:id AND (w.requestedByUserId=:userId OR u.createdBy=:userId OR EXISTS (SELECT 1 FROM PropertyManager pm WHERE pm.propertyId=u.propertyId AND pm.userId=:userId AND pm.active) OR EXISTS (SELECT 1 FROM PropertyOwnership po WHERE po.propertyId=u.propertyId AND (po.unitId IS NULL OR po.unitId=u.id) AND po.homeownerUserId=:userId AND po.active) OR EXISTS (SELECT 1 FROM ProviderService s, ProviderProfile pp WHERE s.id=w.assignedProviderServiceId AND s.profileId=pp.id AND s.active AND pp.active AND pp.userId=:userId))")
 Optional<MaintenanceWorkOrder> findAccessible(long id,long userId);
 @Query("SELECT CASE WHEN COUNT(w)>0 THEN true ELSE false END FROM MaintenanceWorkOrder w, ProviderService s, ProviderProfile pp WHERE w.id=:id AND w.active AND s.id=w.assignedProviderServiceId AND s.profileId=pp.id AND s.active AND pp.active AND pp.userId=:userId")
 boolean isAssignedProvider(long id,long userId);
}
