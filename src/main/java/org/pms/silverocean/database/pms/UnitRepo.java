...
public interface UnitRepo extends JpaRepository<Unit, Long>, JpaSpecificationExecutor<Unit> {
    ...
    @Query("SELECT new org.pms.silverocean.service.property.wrappers.DbUnitDTO(u, p.type, p.typeCategory) FROM Unit u JOIN Property p ON u.propertyId=p.id WHERE u.id=:id AND u.createdBy=:createdBy AND u.active")
    ...
    @Query("SELECT  new org.pms.silverocean.service.property.wrappers.DbUnitDTO(u, p.type, p.typeCategory) FROM Unit u JOIN Property p ON u.propertyId=p.id JOIN Invite li ON u.id=li.entityId WHERE li.type=:inviteType AND li.token=:token AND li.active AND li.expiryDate >= CURRENT_TIMESTAMP AND u.active")
    ...
    @Query("SELECT new org.pms.silverocean.service.property.wrappers.DbUnitDTO(u, p.type, p.typeCategory) FROM Unit u JOIN Property p ON u.propertyId=p.id WHERE u.active AND p.active AND u.id=:id AND " +
            "(p.createdBy=:userId " +
    ...
    @Query("SELECT new org.pms.silverocean.service.property.wrappers.DbUnitDTO(u, p.type, p.typeCategory) FROM Unit u JOIN Property p ON u.propertyId=p.id WHERE u.active AND p.active AND u.id=:id AND EXISTS (SELECT 1 FROM UnitTenant ut WHERE ut.unitId=u.id AND ut.userId=:userId AND ut.active)")
    Optional<DbUnitDTO> findDTOByIdAndTenant(Long id, long userId);

    @Query("SELECT new org.pms.silverocean.service.property.wrappers.DbUnitDTO(u, p.type, p.typeCategory) FROM Unit u JOIN Property p ON u.propertyId=p.id WHERE u.active AND p.active AND u.id=:id AND EXISTS (SELECT 1 FROM PropertyManager pm WHERE pm.propertyId=p.id AND pm.userId=:userId AND pm.roleName=:roleName AND pm.active)")
    ...
    @Query("SELECT new org.pms.silverocean.service.property.wrappers.DbUnitDTO(u, p.type, p.typeCategory) FROM Unit u JOIN Property p ON u.propertyId=p.id WHERE u.active AND p.active AND u.id=:id AND EXISTS (SELECT 1 FROM PropertyOwnership o WHERE o.unitId=u.id AND o.homeownerUserId=:userId AND o.active)")
    Optional<DbUnitDTO> findDTOByIdAndHomeowner(Long id, long userId);

    @Query("SELECT new org.pms.silverocean.service.property.wrappers.DbUnitDTO(u, p.type, p.typeCategory) FROM Unit u JOIN Property p ON u.propertyId=p.id WHERE u.active AND p.active AND u.id=:id AND EXISTS (SELECT 1 FROM SaleTransaction s WHERE s.unitId=u.id AND s.buyerUserId=:userId AND s.active)")
