...
public class SokoController {
    ...
    @GetMapping("/catalog") public ResponseEntity<ResponseDTO> catalog(Pageable pageable,@RequestParam(required=false)Long storeId,@RequestParam(required=false)String category,@RequestParam(required=false)String query,@RequestParam(required=false)Double latitude,@RequestParam(required=false)Double longitude,@RequestParam(required=false)Double radiusKm){var p=service.catalog(pageable,storeId,category,query,latitude,longitude,radiusKm);ResponseDTO r=new ResponseDTO(true,ResponseCode.GENERAL_SUCCESS.getCode(),i18n.getLocalizedMessage(ResponseCode.GENERAL_SUCCESS),p.getContent(),p.getTotalPages(),p.getTotalElements(),p.getSize());return ResponseEntity.ok(r);}
    @GetMapping("/catalog/{storeId}") public ResponseEntity<ResponseDTO> store(@PathVariable long storeId){return ok(service.storeDetail(storeId));}
    ...
    @GetMapping("/order/my") public ResponseEntity<ResponseDTO> myOrders(){return ok(service.myOrders());}
    @GetMapping("/order/merchant") public ResponseEntity<ResponseDTO> merchantOrders(){return ok(service.merchantOrders());}
