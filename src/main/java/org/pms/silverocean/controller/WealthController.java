...
public class WealthController {
    ...
    @GetMapping("/goals/{id}") @PreAuthorize("hasAuthority(T(org.pms.silverocean.service.auth.roles.enums.Permission).VIEW_WEALTH)") public ResponseEntity<ResponseDTO> goalDetails(@PathVariable long id){return ok(service.goalDetails(id));}
    ...
    private ResponseEntity<ResponseDTO> ok(Object data){String description=i18n.getLocalizedMessage(ResponseCode.GENERAL_SUCCESS);return ResponseEntity.ok(data==null?new ResponseDTO(true,ResponseCode.GENERAL_SUCCESS.getCode(),description):new ResponseDTO(true,ResponseCode.GENERAL_SUCCESS.getCode(),description,data));}
}
