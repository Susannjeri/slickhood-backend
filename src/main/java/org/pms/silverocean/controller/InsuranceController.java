...
public class InsuranceController {
    ...
    @GetMapping("/admin/cases/{id}/documents") @PreAuthorize("hasAuthority(T(org.pms.silverocean.service.auth.roles.enums.Permission).REVIEW_INSURANCE_APPLICATIONS)") public ResponseEntity<ResponseDTO> caseDocuments(@PathVariable long id){return ok(operations.caseDocumentsForAgency(id));}
    ...
    @PostMapping("/admin/cases/{id}/assignment-decision") @PreAuthorize("hasAuthority(T(org.pms.silverocean.service.auth.roles.enums.Permission).REVIEW_INSURANCE_APPLICATIONS)") public ResponseEntity<ResponseDTO> assignmentDecision(@PathVariable long id,@Valid @RequestBody org.pms.silverocean.service.insurance.InsuranceModels.AdviserAssignmentDecision r){return ok(operations.decideAssignment(id,r));}
    ...
    @PostMapping(value="/admin/cases/{id}/policy",consumes="multipart/form-data") @PreAuthorize("hasAuthority(T(org.pms.silverocean.service.auth.roles.enums.Permission).ISSUE_INSURANCE_POLICIES)") public ResponseEntity<ResponseDTO> issuePolicy(@PathVariable long id,@Valid @RequestPart("data") org.pms.silverocean.service.insurance.InsuranceModels.PolicyRequest r,@RequestPart("file") MultipartFile file)throws IOException{return ok(operations.issue(id,r,file));}
    ...
    @PostMapping(value="/admin/policies/{id}/renewal-complete",consumes="multipart/form-data") @PreAuthorize("hasAuthority(T(org.pms.silverocean.service.auth.roles.enums.Permission).ISSUE_INSURANCE_POLICIES)") public ResponseEntity<ResponseDTO> renewalComplete(@PathVariable long id,@Valid @RequestPart("data") org.pms.silverocean.service.insurance.InsuranceModels.RenewalCompleteRequest r,@RequestPart("file") MultipartFile file)throws IOException{return ok(renewals.complete(id,r,file));}
    ...
    @PreAuthorize("hasAuthority(T(org.pms.silverocean.service.auth.roles.enums.Permission).MANAGE_INSURANCE_CATALOG)")
    public ResponseEntity<ResponseDTO> deactivateCompany(@PathVariable String code) {
        return ok(service.deactivateCompany(code));
    }
