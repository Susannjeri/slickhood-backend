...
class InsuranceRenewalServiceSecurityTest {
 ...
 @Test void renewedPolicyCannotBeCompletedWithoutItsInsurerSchedule(){InsuranceAgency agency=new InsuranceAgency();agency.setId(1L);InsuranceCase insuranceCase=new InsuranceCase();insuranceCase.setId(9L);insuranceCase.setAgencyId(1L);insuranceCase.setActive(true);InsurancePolicy policy=policy();policy.setRenewalStatus("PAID");InsuranceRenewalOffer offer=offer();offer.setStatus("REMITTED");InsuranceRenewalPayment payment=new InsuranceRenewalPayment();payment.setStatus("REMITTED");when(policies.findByIdForUpdate(4L)).thenReturn(Optional.of(policy));when(cases.findById(9L)).thenReturn(Optional.of(insuranceCase));when(agencies.findByCodeAndActiveTrue("SILVERWOOD")).thenReturn(Optional.of(agency));when(offers.findFirstByPolicyIdAndActiveTrueOrderByCreatedOnDesc(4L)).thenReturn(Optional.of(offer));when(payments.findFirstByRenewalOfferIdAndActiveTrueOrderByCreatedOnDesc(6L)).thenReturn(Optional.of(payment));when(policies.existsByPolicyNumberAndActiveTrueAndIdNot("APA-4-R",4L)).thenReturn(false);assertThatThrownBy(()->service.complete(4L,new InsuranceModels.RenewalCompleteRequest("APA-4-R"),null)).isInstanceOf(PMSCustomException.class);verify(policies,never()).save(any());verify(offers,never()).save(any());verify(documents,never()).save(any());verifyNoInteractions(garage);}
