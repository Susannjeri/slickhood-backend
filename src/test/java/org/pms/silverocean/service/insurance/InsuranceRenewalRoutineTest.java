...
class InsuranceRenewalRoutineTest {
 ...
 @Test void queuesEachExpiryStageOnceWithADeepLink(){InsurancePolicy p=policy(LocalDate.now().plusDays(7));when(policies.findStagedReminderQueue(eq(1L),anyCollection(),any(),any(),any())).thenReturn(new PageImpl<>(List.of(p)));when(policies.findExpiryQueue(eq(1L),any(),any())).thenReturn(Page.empty());when(reminders.existsByPolicyIdAndPolicyEndDateAndReminderDays(4L,p.getEndDate(),7)).thenReturn(false);when(cases.findById(9L)).thenReturn(Optional.of(insuranceCase()));routine.remind();ArgumentCaptor<String> notification=ArgumentCaptor.forClass(String.class);verify(notifications).queueEmailAndInAppOnce(anyString(),eq("client@example.com"),any(),notification.capture(),eq("INSURANCE_RENEWAL"),anyString(),eq("/dashboard/insurance?tab=policies&policyId=4"));assertThat(notification.getValue()).contains("EXPIRES IN 7 DAYS","/dashboard/insurance?tab=policies&amp;policyId=4");verify(reminders).save(argThat(r->r.getPolicyId()==4L&&r.getPolicyEndDate().equals(p.getEndDate())&&r.getReminderDays()==7));}
 ...
 @Test void expiresCoverOnlyAfterTheEndDateAndNotifiesTheCustomer(){LocalDate yesterday=LocalDate.now(ZoneId.of("Africa/Nairobi")).minusDays(1);when(policies.findStagedReminderQueue(eq(1L),anyCollection(),any(),any(),any())).thenReturn(Page.empty());InsurancePolicy p=policy(yesterday);when(policies.findExpiryQueue(eq(1L),eq(yesterday),any())).thenReturn(new PageImpl<>(List.of(p)));when(cases.findById(9L)).thenReturn(Optional.of(insuranceCase()));routine.remind();assertThat(p.getStatus()).isEqualTo("EXPIRED");assertThat(p.getRenewalStatus()).isEqualTo("LAPSED");verify(policies).save(p);verify(notifications).queueEmailAndInAppOnce(anyString(),anyString(),any(),anyString(),eq("INSURANCE_RENEWAL"),anyString(),anyString());}
 @Test void verifiedRenewalPaymentIsNotDowngradedWhilePolicyIssuanceIsPending(){LocalDate yesterday=LocalDate.now(ZoneId.of("Africa/Nairobi")).minusDays(1);when(policies.findStagedReminderQueue(eq(1L),anyCollection(),any(),any(),any())).thenReturn(Page.empty());InsurancePolicy p=policy(yesterday);p.setRenewalStatus("PAID");when(policies.findExpiryQueue(eq(1L),eq(yesterday),any())).thenReturn(new PageImpl<>(List.of(p)));when(cases.findById(9L)).thenReturn(Optional.of(insuranceCase()));routine.remind();assertThat(p.getStatus()).isEqualTo("EXPIRED");assertThat(p.getRenewalStatus()).isEqualTo("PAID");verify(policies).save(p);}
