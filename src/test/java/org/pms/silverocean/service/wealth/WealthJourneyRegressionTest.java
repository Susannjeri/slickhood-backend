...
class WealthJourneyRegressionTest {
 ...
 @Test void serviceRejectsAPastGoalDateEvenWhenCalledOutsideTheController(){var yesterday=LocalDate.now(PMSUtils.getZoneId()).minusDays(1);assertThatThrownBy(()->service.addGoal(new WealthRequests.GoalRequest("NET_WORTH","Expired target",BigDecimal.TEN,"KES",yesterday))).isInstanceOf(PMSCustomException.class);verify(goalRepo,never()).save(any());}
