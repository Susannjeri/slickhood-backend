...
- Migration V47 adds report-specific date indexes to keep bounded queries efficient as operational tables grow. The audited release rebased the original V52 candidate onto production's current V46 baseline so future migrations cannot be skipped.

## Staging release checks

1. Deploy the backend and confirm Flyway applies V47 successfully. On a production-sized clone, record migration duration and inspect the slow-query log during each index build.
...
- If V47 causes unacceptable migration locking, stop promotion and restore the previous application release. Do not remove a successfully built index during peak traffic; schedule any rollback migration separately.
